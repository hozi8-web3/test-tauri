...
const SettingsInput = ({ value, onChange, type = 'text', placeholder = '', step }: {
...
export const SettingsPage = () => {
    ...
    const loadSettings = useCallback(async () => {
        ...
        if (res.success && res.rows) {
            ;(res.rows as unknown as SettingRow[]).forEach((r) => {
    ...
    const handleChangePin = async (e: React.FormEvent) => {
        ...
        const { row } = await window.api.db.get("SELECT id, pin_hash FROM users WHERE role = 'Admin' LIMIT 1")
        if (row?.pin_hash) {
            const matchRes = await window.api.auth.compare(oldPin, row.pin_hash as string)
            if (matchRes.match) {
                const hashRes = await window.api.auth.hash(newPin)
                if (hashRes.success && hashRes.hash) {
                    await window.api.db.run("UPDATE users SET pin_hash = ? WHERE role = 'Admin'", [hashRes.hash])
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <form onSubmit={handleSaveInfo} className="p-4 space-y-3">
                            <LabelField label="Store Name"><SettingsInput value={storeName} onChange={setStoreName} placeholder="Al-Barkat Mart" /></LabelField>
                            <LabelField label="Address Line 1"><SettingsInput value={address1} onChange={setAddress1} placeholder="Shop 1, Main Market" /></LabelField>
                            <LabelField label="Address Line 2"><SettingsInput value={address2} onChange={setAddress2} placeholder="Lahore, Pakistan" /></LabelField>
                            <LabelField label="Phone Number"><SettingsInput value={phone} onChange={setPhone} placeholder="+92-300-1234567" /></LabelField>
                            <LabelField label="Default Tax Rate (%)"><SettingsInput type="number" step="0.1" value={taxRate} onChange={setTaxRate} /></LabelField>
